...
for noise=0:0.01:1
    ...
    for j=1:4
        ...
        switch j
            case 1
                hold on
                hh=plot(noise,real(v),'r*');
                ylabel('real');
            case 2
                hold on
                hh=plot(noise,imag(v),'r*');
                ylabel('imaginary');
            case 3
                hold on
                hh=plot(noise,abs(v),'r*');
                ylabel('abs');
            case 4
                hold on
                hh=plot(noise,angle(v),'r*');
                ylabel('angle');
        end
    end
...
suptitle({str1});
saveas(gcf,['.\Logistic_lambda_noise\Logistic_lambda_noise_',setup.function,'_n',num2str(n),'_m',num2str(m),'.png'])
...
%     suptitle({str1;str2});
%     str=['.\Logistic_saparse_noise\Logistic_',setup.function,'_',setup.complex,'_n',num2str(n),'m',num2str(m),'_figure',num2str(i)];
%     saveas(hh,[str,'.png'])
%     saveas(hh,[str,'.fig'])
% end