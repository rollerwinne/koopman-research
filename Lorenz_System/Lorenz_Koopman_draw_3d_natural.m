clear;clc;close all;tic;timestart=char(datetime('now'));
...
x_k=x(1:end-1,:);
x_l=x(2:end,:);
[F,D,x_function_length,dj]=Lorenz_Koopman_U_natural(x_k,x_l);
...
for i=1:min(4,length(h))
    ...
    str1=['n=',num2str(length(x_k(1:end,1))),'; m=',num2str(x_function_length),'; d_{j}=',num2str(dj),''];
