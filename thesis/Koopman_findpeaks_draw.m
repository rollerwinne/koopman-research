...
s=jet(n);YY=[];
M=[2];%,3,4,5,8,10,15,20];%,14,16,20,28,32,38,44,50,60,64,72,80,90,100];
...
%saveas(gcf,['./temp5/Tent_noise_phase_d',num2str(d),'.png']);
for m=M
    figure(m);
    ...
    for i=1:min(length(h),9)
        ...
        str1=['m=',num2str(m)];
        str2=[' \lambda=',num2str(d_abs),'∠' num2str(d_angle),'°'];
        title([str1,str2]);
    end
    suptitle(['Eigenfunctions of Tent Map with noise (n=',num2str(n),',noise=',num2str(d),')']);
    filename=['Tent_eigen_noise_n1000m',num2str(m),'d',num2str(d),'.png'];
    %saveas(hh,['./temp5/',filename]);
    %YY=[YY,AA-mean(AA)];
end
colormap(jet)
%figure(102)
