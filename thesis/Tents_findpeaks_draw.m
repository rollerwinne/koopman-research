...
%saveas(gcf,['./temp/Tents5l_phase_d',num2str(d),'.png']);
cal=1;
figure;
for m=M
    ...
    for i=choose%1:min(length(h),9)
        ...
        if exist('seq')%边界点
            s=jet(4);
            drawBoundary(seq,(min(A)+max(A))/2,s(1,:));
            for ii=1:MM(cal-1)
                drawBoundary(sx(ii,:),(min(A)+max(A))/2,s(ii+1,:));
            end
...
filename=['Tents52_eigen_Gauss_n1000_m',toString(M,'-'),'_d',num2str(d)];
savesci(filename);
...
function drawBoundary(X,yt,color)
size=10;rate=0.87;
for j=1:length(X(:,1))
    x=X(j,:);
    plot(x,yt,'bo','MarkerSize',size*rate^3,'MarkerFaceColor',color);%,'Color',s(j-m+1,:)
end
end
