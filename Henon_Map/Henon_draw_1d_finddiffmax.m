...
for i=1:n
    ...
    for j=1:min(nn,length(h))
        ...
        F_X=F_start:q;
        F_temp=abs(F(:,h(j)));
        %F_findnum=floor(length(F_temp)*percent);
        F_diff=diff(F_temp);
        F_diff2=-diff(F_temp);
        F_which=[0;F_diff(1:end-1)]>0.005 & [0;F_diff2(2:end)]>0.005;
        %[~,F_index]=sort(F_temp);
...
str1=['.\temp\Henon_eigenfunctions1d_finddiffmax_complex_figure.fig'];
str2=['.\temp\Henon_eigenfunctions1d_finddiffmax_complex_figure.png'];
%if ~isempty(h)
saveas(hh,str1);
saveas(hh,str2);
%end
